"""Engine adapters over identical graph input for a fair three-way comparison.

Build the *same* graph in AnnNet, NetworkX and igraph, and expose the
semantically-comparable operations behind one interface.

Fairness rules baked in here:

* All three engines are fed an identical vertex list and ``(src, tgt, weight)``
...
* Query closures perform a single representative call (one ``degree``, one
  ...
  (NetworkX), a C adjacency (igraph) and an incidence matrix (AnnNet) — from the
  degree of any particular vertex (the ring input keeps degree constant across
  scale on purpose).
...
def baseline_engines(directed: bool = True) -> list[Engine]:
    return [NetworkXEngine(directed=directed), IGraphEngine(directed=directed)]
