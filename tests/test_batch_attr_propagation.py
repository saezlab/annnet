...
The single-node / single-edge forms accept and persist arbitrary user
...
# ── add_nodes: list-form kwargs broadcast ──────────────────────────────


def test_add_nodes_list_kwargs_broadcasts_attrs():
    """Calling add_nodes with a list and kwargs must broadcast the
    kwargs across every listed node (matches single-string semantics)."""
    G = AnnNet(directed=False)
    G.add_nodes(['A', 'B', 'C'], kind='gene', expression=1.5)
    for vid in ('A', 'B', 'C'):
        assert _node_attr_value(G, vid, 'kind') == 'gene'
        assert _node_attr_value(G, vid, 'expression') == 1.5


def test_add_nodes_dict_batch_preserves_attrs():
    G = AnnNet(directed=False)
    G.add_nodes(
        [
            {'node_id': 'A', 'kind': 'gene'},
    ...
    assert _node_attr_value(G, 'A', 'kind') == 'gene'
    assert _node_attr_value(G, 'B', 'kind') == 'protein'


def test_add_nodes_per_item_attrs_override_kwargs():
    """Per-item attrs in dict form take precedence over batch kwargs."""
    G = AnnNet(directed=False)
    G.add_nodes(
        [
            {'node_id': 'A', 'kind': 'protein'},
            {'node_id': 'B'},
        ],
        kind='gene',
    )
    assert _node_attr_value(G, 'A', 'kind') == 'protein'
    assert _node_attr_value(G, 'B', 'kind') == 'gene'
