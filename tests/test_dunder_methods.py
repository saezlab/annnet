...
`len(G)` and `for v in G` follow the NetworkX convention (node iteration).
`in` is node-membership only — edges are not supported as a `__contains__`
...
def test_len_returns_node_count():
