...
class LighAnnNet(AttributesClass, IndexMapping):
    ...
    ----------
    entity_to_idx : dict
        Mapping of entity ID (Identifier) to row index in the incidence matrix.
    edge_to_idx : dict
    ...
    edge_attributes : DataFrame
        Table storing metadata for edges.
    ...
    def __init__(
        ...
        self._matrix = sp.dok_matrix((int(n), int(e)), dtype=np.float32)
        self._num_entities = 0
        self._num_edges = 0

        # Matrix resizing helpers
        self._grow_rows_to = lambda target: self._resize_matrix(target, None)
        self._grow_cols_to = lambda target: self._resize_matrix(None, target)

        # Annotation Tables
        self._annotations_backend = annotations_backend
        self._init_annotation_tables(annotations)

    def _resize_matrix(self, rows, cols):
        curr_r, curr_c = self._matrix.shape
        new_r = max(rows or 0, curr_r + max(8, curr_r >> 1)) if rows else curr_r
        new_c = max(cols or 0, curr_c + max(8, curr_c >> 1)) if cols else curr_c
        if new_r > curr_r or new_c > curr_c:
            self._matrix.resize((new_r, new_c))
